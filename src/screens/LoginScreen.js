...
const LoginScreen = props => {
  ...
  useEffect(() => {
    if (props.user.error !== null && props.user.error !== undefined) {
      setIsLoading(false);
      setIsError(true);
    } else if (props.user.data !== null && props.user.data !== undefined) {
      setIsLoading(false);
  ...
  useEffect(() => {
    if (isError) {
      Alert.alert(
        '',
        'You email or password is incorrect. Please try again',
        [
          {
            text: 'Ok',
            onPress: () => handleReset(),
